import { rankingFromHistory, victories } from '../game/online-stats.js?v=20260910T004553338';
import { playerKey } from '../game/engine.js?v=20260910T004553338';
...
export function averageBlockedWinPoints(history, playerId) {
  const values = [];
  for (const match of history || []) {
    const rounds = Array.isArray(match?.rounds) ? match.rounds : Object.values(match?.rounds || {});
    for (const round of rounds) {
      if (!round || round.type !== 'winner' || round.cause !== 'blocked' || String(round.winnerId) !== String(playerId)) continue;
      const value = Number(round.points ?? round.handPoints?.[playerKey(playerId)]);
