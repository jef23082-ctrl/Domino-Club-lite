...
export function finishRoundInRoom(room, winnerId, cause, details = {}, at = now()) {
  ...
  const remainingHands = Object.fromEntries(
    (game.playerOrder || []).map(id => [playerKey(id), [...(game.hands?.[playerKey(id)] || [])]])
  );
  const handPointTotals = Object.fromEntries(
    (game.playerOrder || []).map(id => [playerKey(id), handPoints(remainingHands[playerKey(id)])])
  );
  const lastTileId = cause === 'empty'
    ? details.lastTileId || [...(game.board?.placements || [])]
      .sort((left, right) => Number(right.playedAt || 0) - Number(left.playedAt || 0))[0]?.tileId || null
    : null;
  ...
  game.roundResult = {
    ...
    handPoints: details.handPoints || handPointTotals,
    remainingHands,
    lastTileId,
    at
  };
...
export function cancelRoundInRoom(room, reason, handPointTotals = {}, at = now()) {
  ...
  const totals = Object.keys(handPointTotals || {}).length
    ? handPointTotals
    : Object.fromEntries((game.playerOrder || []).map(id => [playerKey(id), handPoints(remainingHands[playerKey(id)])]));
  game.roundStatus = 'ended';
  game.roundResult = { type: 'annulled', reason, handPoints: totals, remainingHands, lastTileId: null, at };
  room.roundLog = room.roundLog || [];
  room.roundLog.push({
    roundNumber: game.roundNumber,
    type: 'annulled',
