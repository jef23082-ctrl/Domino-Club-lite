import { element as el, button } from './club-elements.js?v=20260910T004553338';
import { renderChatMessage } from './chat-renderer.js?v=20260910T004553338';
import { playSound } from './sound-player.js?v=20260910T004553338';
...
export function createChatComposer({ repository, channel, identity, canWrite, notify, audible=()=>true, title = 'Discussion commune' }) {
  ...
  const form=el('form','portal-chat-form'), input=el('input','online-input');input.maxLength=300;input.placeholder='Écrire à tous les joueurs…';input.setAttribute('aria-label','Message commun');
  ...
  const stop=repository.watch(channel,{added:m=>{entries.set(m.id,m);render();if(audible() && Number(m.createdAt)>=boundAt && m.senderToken!==identity().clientToken)playSound('message');},changed:m=>{entries.set(m.id,m);render();},removed:id=>{entries.delete(id);render();},error:e=>notify(e.message,'error')});
  let typingEntries={};const renderTyping=()=>{typing.textContent=Object.entries(typingEntries).filter(([token,item])=>token!==identity().clientToken && Date.now()-Number(item.at)<4500).map(([,item])=>displayName(item.name)).join(', ');if(typing.textContent)typing.textContent+=' écrit…';};
