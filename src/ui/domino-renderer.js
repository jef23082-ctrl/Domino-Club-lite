import { parseTile } from '../game/engine.js?v=20260906T181957527';
import { displayName } from '../online/display-name.js?v=20260906T181957527';
import { createWorkBeacon } from './work-beacon.js?v=20260906T181957527';
...
export function createPremiumDomino(tileId, className = '') {
  const tile = parseTile(tileId);
  const domino = document.createElement('span');
  domino.className = `result-domino ${className}`.trim();
  domino.dataset.tileId = tileId;
  domino.setAttribute('role', 'img');
  domino.setAttribute('aria-label', `Domino ${tile.a}-${tile.b}`);
  domino.append(createHandHalf(tile.a), createHandHalf(tile.b));
  return domino;
}
...
export function renderPlayerPlaque(container, player) {
  ...
  renderPlayerWorkStatus(container, player);
}

export function renderPlayerWorkStatus(container, player) {
  container.classList.toggle('is-working', Boolean(player.working));
  container.querySelector('.player-work-status')?.remove();
