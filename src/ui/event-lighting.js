...
export function applyEventLighting(stage, room, options) {
  ...
  if (stage) {
    ...
    else if (!stage.classList.contains('is-boude-lighting')) delete stage.dataset.spectacularEvent;
  }
  return mode;
}
...
export function pulseBoudeLighting(stage, duration = 2800) {
  ...
  setTimeout(() => {
    stage.classList.remove('is-boude-lighting');
    if (stage.dataset.spectacularEvent === 'boude') delete stage.dataset.spectacularEvent;
  }, duration);
}
