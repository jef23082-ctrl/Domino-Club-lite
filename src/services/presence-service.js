...
export class PresenceService {
  ...
  updateRoom(roomCode = '', role = 'lobby') {
    this.roomCode = roomCode;
    this.role = role;
    if (!roomCode) this.working = false;
  ...
  setWorking(active = false) {
    this.working = Boolean(active);
    if (!this.connected || !this.reference) return Promise.resolve(false);
    return this.reference.update({
      working: this.working,
      lastSeen: this.serverTimestamp()
    }).then(() => true);
  }
  ...
  async disconnect() {
    ...
    this.timer = null;
    if (!this.reference) { this.working = false; return; }
